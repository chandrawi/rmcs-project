...
if len(sys.argv) > 1:
    ...
    group_id = config.INCLINOMETER_ARRAY_GROUP_ID
...
models = resource.list_model_by_name("3-axis 16-bit inclinometer & thermometer")
model_raw = models[0]
models = resource.list_model_by_name("SCL3300 YZ-axis inclinometer array")
model_data = models[0]

# get soil inclinometer group and devices
devices_map: List[DeviceMap] = []
group = resource.read_group_device(UUID(group_id))
if group.name == "SCL3300 inclinometer array":
    ...
    for device_id in group.devices:
        ...
        for conf in device.configs:
            if conf.name == "offset-X":
                ...
                position = conf.value
            elif conf.name == "initial_displacement-Y":
                initial_displacement_y = conf.value
        ...
        if offset_x != None and offset_y != None and offset_z != None and space != None and position != None:
            device_map = DeviceMap(device_id, device.serial_number, offset_x, offset_y, offset_z, position, space, initial_displacement_y, initial_displacement_z)
...
while True:
    ...
    for i in range(number):
        angle_x = get_i16_value(buffer_set[i].data[0], device_set[i].offset_x) * (math.pi / 2) / 16384
        angle_y = get_i16_value(buffer_set[i].data[1], device_set[i].offset_y) * (math.pi / 2) / 16384
        angle_z = get_i16_value(buffer_set[i].data[2], device_set[i].offset_z) * (math.pi / 2) / 16384
        temperature = (get_i16_value(buffer_set[i].data[3], 0) / 18.9) - 273

        if i > 0:
            displacement_y = device_set[i].space * ((math.sin(last_angle_y) + math.sin(angle_y)) / 2) * -1
            displacement_z = device_set[i].space * ((math.sin(last_angle_z) + math.sin(angle_z)) / 2)
        ...
        data = [angle_y, angle_z, displacement_y_total, displacement_z_total, temperature]
        try:
